...
BASE_FOLDER = os.path.join(CURRENT_DIR, "NAJMI_ADMIN-ERA")
...
def save_rare_account(account_data, rarity_type, reason, rarity_score, is_ghost=False):
    try:
        ...
        rare_entry = {
            ...
            'region': "NAJMI_ADMIN" if is_ghost else account_data.get('region', 'UNKNOWN'),
...
def save_couples_account(account1, account2, reason, is_ghost=False):
    try:
        ...
        couples_entry = {
            ...
            'reason': reason,
            'region': "NAJMI_ADMIN" if is_ghost else account1.get('region', 'UNKNOWN'),
...
def save_jwt_token(account_data, jwt_token, region, is_ghost=False):
    try:
        ...
        token_entry = {
            ...
            'region': "NAJMI_ADMIN" if is_ghost else region,
            'thread_id': account_data.get('thread_id', 'N/A')
        }
    ...
    while True:
        try:
            ...
            if choice == "00":
                ...
                print(f"\n{get_random_color()}{Colors.BRIGHT}👋 Thank you for using NAJMI_ADMIN Generator!{Colors.RESET}")
                sys.exit(0)
            elif choice.isdigit():
...
def about_section():
    ...
    print(f"{get_random_color()}{Colors.BRIGHT}ℹ️  About NAJMI_ADMIN Account Generator{Colors.RESET}")
...
def main_menu():
    while True:
        ...
        print(f"{get_random_color()}{Colors.BRIGHT}🎮 Welcome to NAJMI_ADMIN Account Generator{Colors.RESET}")
